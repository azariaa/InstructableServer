...
public class FieldChanged implements IFieldChanged
{
    ...
    public void fieldChanged(JSONObject fieldVal)
    {
        ...
        try (
                ...
                PreparedStatement pstmt = connection.prepareStatement("insert into "+SingleInstance.instanceValTableName+" (" + SingleInstance.userIdColName + "," + SingleInstance.conceptColName + "," + SingleInstance.instanceColName + "," + SingleInstance.fieldColName + "," + SingleInstance.fieldJSonValColName + ") values (?,?,?,?,?) on duplicate key update " + SingleInstance.fieldJSonValColName + "=?");
        )
        {
